...
import {
  Activity, BarChart2, TrendingUp, Network, Scale, BarChart3,
} from 'lucide-react';
...
export interface MarketRegimeData {
  ...
  duration: string;
  indicators: {
    vix: { value: number | string; label: string; status: string };
    breadth: { value: number | string; label: string; status: string };
    trend: { value: number | string; label: string; status: string };
    correlation: { value: number | string; label: string; status: string };
    putCall: { value: number | string; label: string; status: string };
  };
  commentary: string;
}
...
type IndicatorKey = keyof MarketRegimeData['indicators'];

const indicatorConfig: Record<IndicatorKey, { icon: React.ElementType; name: string }> = {
  ...
  return (
    <GlassCard className={cn('h-full flex flex-col relative overflow-hidden', className)} variant="dark">
      ...
      <div className="relative z-10 space-y-6">
        {/* Header */}
        <div>
          <h3 className="text-xs font-mono uppercase tracking-widest text-turquoise-mist mb-2">
            Current Assessment
          </h3>
        ...
        <div className="grid grid-cols-2 gap-2">
          {(Object.entries(data.indicators) as [IndicatorKey, MarketRegimeData['indicators'][IndicatorKey]][]).map(([key, indicator]) => {
            const config = indicatorConfig[key];
            const Icon = config?.icon ?? Activity;

            const statusDotClass = indicator.status === 'good'
              ? 'bg-gain text-gain'
            ...
            return (
              <div key={key} className="p-2.5 rounded-lg bg-white/5 border border-white/10 flex flex-col gap-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <div className={cn('w-1.5 h-1.5 rounded-full shadow-[0_0_5px_currentColor]', statusDotClass)} />
                </div>

                <div>
                  <div className="font-mono font-bold text-sm text-paper-100">{indicator.value}</div>
                  <div className="text-[10px] text-turquoise-mist truncate">{indicator.label}</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="pt-4 border-t border-white/10">
          <h4 className="text-xs font-mono uppercase tracking-widest text-turquoise-mist mb-2">
            Curator Commentary
          </h4>
