...
const statusConfig: Record<StrategyStatus, { color: string; text: string; label: string; pulse: boolean }> = {
...
const typeConfig: Record<StrategyType, { label: string; icon: React.ElementType }> = {
  ...
  macro: { label: 'Macro', icon: AlertTriangle },
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex h-2.5 w-2.5">
              {statusInfo.pulse && (
                <span className={cn('animate-ping absolute inline-flex h-full w-full rounded-full opacity-75', statusInfo.color)} />
              )}
              <span className={cn('relative inline-flex rounded-full h-2.5 w-2.5', statusInfo.color)} />
            </div>
