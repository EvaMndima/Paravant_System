...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, containerClassName, type = "text", label, error, helperText, leftIcon, rightIcon, disabled, ...props }, ref) => {
    ...
    return (
      <div className={cn("space-y-1.5 w-full", containerClassName)}>
        ...
        >
          {leftIcon && <div className="pl-3 text-obsidian-400/40 dark:text-paper-100/40 select-none">{leftIcon}</div>}
          <input
            ref={ref}
            type={inputType}
            disabled={disabled}
        ...
        {(error || helperText) && (
          <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-1.5 ml-1 h-4">
            {error && <AlertCircle className="w-3 h-3 text-loss" />}
            <span className={cn("text-xs font-medium tracking-wide", error ? "text-loss" : "text-obsidian-400/50 dark:text-paper-100/50")}>
              {error || helperText}
            </span>
          </motion.div>
        )}
