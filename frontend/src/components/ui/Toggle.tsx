...
const Toggle = React.forwardRef<HTMLButtonElement, ToggleProps>(
  ({ checked, onCheckedChange, size = 'md', disabled = false, className, style, ...props }, ref) => {
    const dimensions = { sm: { w: 36, h: 20, p: 2 }, md: { w: 44, h: 24, p: 2 } };
    ...
    return (
      ...
      >
        <motion.div
          ...
          animate={{ x: checked ? currentDim.w - currentDim.h : 0 }}
          style={{ width: thumbSize, height: thumbSize, margin: currentDim.p }}
        />
      </motion.button>
    );
  }
