...
// --- Route mapping ---
// Maps sidebar labels to routes and vice versa
...
export function getLabelFromPath(pathname: string): string {
  return ROUTE_TO_LABEL[pathname] || 'Cockpit';
}
...
export const Sidebar: React.FC = () => {
  ...
  const activeTab = getLabelFromPath(location.pathname);
  ...
  const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => {
    // Force expanded on mobile so the drawer always shows labels
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className={cn(
          ...
          collapsed ? "justify-center px-0 h-24 pt-4" : "justify-between h-20"
        )}>
          <div className={cn(
        ...
        {/* Footer */}
        <div className={cn("p-3 space-y-4", collapsed && "pb-5")}>
          ...
          <div className={cn("flex flex-col gap-4", collapsed ? "items-center" : "px-2")}>
            ...
            <div className={cn(
              ...
              collapsed && "justify-center p-1.5 hover:bg-transparent"
            )}>
              <Avatar
