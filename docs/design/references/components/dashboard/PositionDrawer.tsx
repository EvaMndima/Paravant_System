...
// Re-using the structure from PositionsTable for consistency, extending it slightly for the detail view
interface Holding {
  id: string;
  symbol: string;
  name: string;
...
const DetailRow = ({ label, value, subValue, highlight = false }: { label: string, value: string, subValue?: string, highlight?: boolean }) => (
  <div className="flex flex-col">
    <span className="text-[10px] uppercase tracking-widest text-obsidian-400/50 dark:text-paper-100/50 font-mono mb-1">{label}</span>
    <span className={cn("text-base font-medium font-mono", highlight ? "text-deep-teal-800 dark:text-turquoise-mist" : "text-obsidian-400 dark:text-paper-100")}>
      {value}
    </span>
    {subValue && <span className="text-xs text-obsidian-400/60 dark:text-paper-100/60">{subValue}</span>}
  </div>
);
...
export const PositionDrawer: React.FC<PositionDrawerProps> = ({
  ...
  // Generate chart data only when position changes
            ...
            <div className="flex-shrink-0 px-6 py-5 border-b border-deep-teal-800/5 dark:border-white/5 flex items-start justify-between bg-white/50 dark:bg-black/10">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <Badge variant="neutral" size="sm" className="font-normal opacity-80">{position.assetType}</Badge>
                </div>
              ...
              <div className="flex flex-col items-end">
                 ...
                 {/* Mock Day Change since Holding doesn't usually have it */}
                 <div className={cn("flex items-center gap-1 text-xs font-bold font-mono", position.pnlPercent >= 0 ? "text-gain" : "text-loss")}>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              ...
              <div className="p-6 grid grid-cols-2 gap-y-6 gap-x-4">
                 ...
                 <DetailRow 
                   ...
                   highlight={position.pnl >= 0} // Could conditionally color based on gain/loss
                 />
                 <DetailRow label="Portfolio Weight" value={`${position.weight.toFixed(2)}%`} />
                 <DetailRow label="Day P&L" value={formatCurrency(position.value * 0.012)} subValue="+1.20%" />
              </div>

              {/* Chart Section */}
              <div className="px-6 py-2">
                 <div className="text-xs font-mono uppercase tracking-widest text-obsidian-400/50 dark:text-paper-100/50 mb-3">Performance (30D)</div>
              ...
              <div className="px-6 py-6">
                 <div className="flex items-center justify-between mb-3">
                    <div className="text-xs font-mono uppercase tracking-widest text-obsidian-400/50 dark:text-paper-100/50">Recent History</div>
                       ...
                       <div key={tx.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-deep-teal-800/5 dark:hover:bg-white/5 transition-colors">
                          <div className="flex items-center gap-3">
                             <div className={cn(
                               ...
                               tx.type === 'Buy' ? "bg-gain/10 border-gain/20 text-gain" : "bg-loss/10 border-loss/20 text-loss"
                             )}>
                                {tx.type === 'Buy' ? <DollarSign className="w-4 h-4" /> : <ArrowRightLeft className="w-4 h-4" />}
                             </div>
                             <div>
                          ...
                          <div className="text-right">
                             ...
                             <div className="text-[10px] text-obsidian-400/50 dark:text-paper-100/50 font-mono">Total: {formatCurrency(tx.total)}</div>
                          </div>
                       </div>
                    ))}
                 </div>
              ...
              <div className="px-6 pb-6">
                 <div className="text-xs font-mono uppercase tracking-widest text-obsidian-400/50 dark:text-paper-100/50 mb-3">Fundamentals</div>
                 <div className="grid grid-cols-2 gap-2">
                    <div className="p-3 bg-deep-teal-800/5 dark:bg-white/5 rounded-lg border border-deep-teal-800/5 dark:border-white/5">
                       <div className="text-[10px] opacity-60 mb-1">Market Cap</div>
                       <div className="font-mono text-sm font-medium">2.24T</div>
                    </div>
                    <div className="p-3 bg-deep-teal-800/5 dark:bg-white/5 rounded-lg border border-deep-teal-800/5 dark:border-white/5">
                       <div className="text-[10px] opacity-60 mb-1">P/E Ratio</div>
                       <div className="font-mono text-sm font-medium">74.2</div>
                    </div>
                    <div className="p-3 bg-deep-teal-800/5 dark:bg-white/5 rounded-lg border border-deep-teal-800/5 dark:border-white/5">
                       <div className="text-[10px] opacity-60 mb-1">Div Yield</div>
                       <div className="font-mono text-sm font-medium">0.02%</div>
                    </div>
                    <div className="p-3 bg-deep-teal-800/5 dark:bg-white/5 rounded-lg border border-deep-teal-800/5 dark:border-white/5">
                       <div className="text-[10px] opacity-60 mb-1">52W Range</div>
                       <div className="font-mono text-sm font-medium">400 - 950</div>
                    </div>
                 </div>
              </div>
              ...
              <div className="px-6 pb-8">
                 <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="text-xs font-mono uppercase tracking-widest text-obsidian-400/50 dark:text-paper-100/50">Personal Notes</div>
                 </div>
                 <div className="relative">
                    ...
                    <div className="absolute bottom-3 right-3">
                       <Button size="sm" variant="ghost" disabled={!note} className="h-7 text-xs bg-white/50 dark:bg-black/20 hover:bg-white dark:hover:bg-black/40">
                          Save
                       </Button>
                    </div>
                 </div>
