...
export interface Position {
  ...
  plPercent: number;
  weight: number;
  sector?: string; // Optional for compatibility with Cockpit data
...
export const PositionsTable: React.FC<PositionsTableProps> = ({ 
  ...
  const displayData = data ? (limit ? data.slice(0, limit) : data) : (limit ? mockPositions.slice(0, limit) : mockPositions);
  ...
  const columns: Column<Position>[] = [
    ...
    },
    {
      ...
      render: (val) => <span className="font-mono text-obsidian-400/80 dark:text-paper-100/80">{formatNumber(val)}</span>
    },
    {
      key: 'avgPrice',
      header: 'Avg Price',
      ...
      render: (val) => <span className="font-mono text-obsidian-400/60 dark:text-paper-100/60">{formatCurrency(val)}</span>
    },
    {
      key: 'currentPrice',
      header: 'Mark',
    ...
    },
    {
      ...
      render: (val) => {
          ...
          const Icon = val === 0 ? Minus : (isPositive ? ArrowUpRight : ArrowDownRight);
          return (
            <div className={cn(
                ...
                <span>{Math.abs(val).toFixed(2)}%</span>
            </div>
          );
      }
    },
    {
        ...
        render: (val) => (
            <div className="flex items-center justify-end gap-2">
                <span className="font-mono text-xs text-obsidian-400/70 dark:text-paper-100/70">{val.toFixed(1)}%</span>
