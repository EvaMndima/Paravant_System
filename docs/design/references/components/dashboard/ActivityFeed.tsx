...
import { smoothSpring, staggerContainer, fadeInUp } from '../../lib/animations';
...
const getTypeConfig = (type: ActivityType) => {
  switch (type) {
    ...
    case 'agent':
      return { icon: Bot, color: 'text-deep-teal-800 dark:text-turquoise-mist', borderColor: 'border-deep-teal-800/30 dark:border-white/30' };
    default:
...
export const ActivityFeed: React.FC<ActivityFeedProps> = ({
  ...
  const [filter, setFilter] = useState<'all' | 'trades' | 'alerts' | 'transfers'>('all');
  ...
  const filteredItems = useMemo(() => {
    ...
    return result.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime()).slice(0, maxItems);
                ...
                return (
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      // Solid background to hide timeline line
                      "bg-paper-100 dark:bg-obsidian-300",
                      // Ring to blend with background if needed
                          ...
                          {item.metadata && (
                            <div className="flex flex-wrap gap-2 mt-2">
                              {Object.entries(item.metadata).map(([key, value]) => (
                                ...
                                >
                                  <span className="opacity-50 mr-1 uppercase">{key}:</span> {value}
                                </span>
                              ))}
                            </div>
                          )}
