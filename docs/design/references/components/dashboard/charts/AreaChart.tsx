...
export const AreaChart: React.FC<AreaChartProps> = ({
  ...
  const getThemeColor = () => {
    if (isDark) {
        ...
        switch (appTheme) {
          case 'sapphire': return '#60A5FA'; // Blue 400
          case 'emerald': return '#34D399'; // Emerald 400
          case 'onyx': return '#FCD34D'; // Amber 300 (Gold)
          case 'ocean':
          default: return '#48CAE4'; // Bright Cyan/Turquoise
        }
    }
  ...
  useEffect(() => {
    // Diagnostic log
    console.log(`AreaChart mounted. Data points: ${data?.length}`);
  }, [data]);
  ...
  const FallbackChart = () => {
    ...
    // Create points string "x,y x,y" for SVG polyline
