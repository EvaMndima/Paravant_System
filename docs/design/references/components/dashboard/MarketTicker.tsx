...
// Sub-component to handle individual flash logic
    ...
    >
      ...
      >
        <div className={cn("flex animate-marquee", durationClass)}>
          {items.map((item, i) => <TickerItem key={`a-${item.symbol}-${i}`} item={item} />)}
        </div>
        <div className={cn("flex animate-marquee", durationClass)} aria-hidden="true">
          {items.map((item, i) => <TickerItem key={`b-${item.symbol}-${i}`} item={item} />)}
        </div>
      </div>
